...
from .utils import iter_markdown_files, process_file
...
def compute_priority(load, force, necessity, value, due):
    params = {
        'alpha': 1.5, 'beta': 2.0, 'gamma': 2.5, 'delta': 1.5,
        'w_L': 0.4, 'w_F': 0.8, 'k': 5.0, 'theta': 0.75, 'max_days': 60
    }
...
def update_priority(line, pr):
    line = re.sub(r'\[priority::.*?\]', '', line).strip()
    return f"{line} [priority:: {pr}]\n"
...
def process_line(line, reset):
    ...
    if not reset and "[priority::" in line:
        return line
